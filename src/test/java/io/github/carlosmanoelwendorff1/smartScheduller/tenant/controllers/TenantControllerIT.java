...
class TenantControllerIT extends AbstractIntegrationTest {
    ...
    /**
     * Tenant admin endpoints aren't scoped by TenantContext (they operate ON
     * a tenant, identified by path {id}, not FOR the caller's own tenant),
     * so any valid, authenticated token works here - the tenantId inside it
     * is irrelevant for these particular tests.
     */
    private String bearerToken() {
        return "Bearer " + jwtService.generateToken(UUID.randomUUID(), UUID.randomUUID(), "ADMIN");
    }

    @Test
    void createsTenantAndReturns201WithLocationHeader() throws Exception {
        // POST /api/v1/tenants is the bootstrap endpoint - stays open, no token needed.
